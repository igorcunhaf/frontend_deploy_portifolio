...
// import { getCookie } from 'cookies-next'; // Removido - Não precisa mais de token
...
export default function IntranetPage() {
  ...
  const [loadingMessages, setLoadingMessages] = useState(false); // Estado para indicar carregamento
  const [errorMessages, setErrorMessages] = useState<string | null>(null); // Estado para erros
  ...
  // Função para buscar mensagens (sem JWT)
  const fetchMessages = async () => {
    setLoadingMessages(true);
    setErrorMessages(null);
    try {
      // Requisição simples sem cabeçalho de autorização
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/contact`);

      if (!res.ok) {
        // Captura o status e a mensagem de erro se possível
  ...
  // Funções de delete ainda precisam de token, mas vamos mantê-las por enquanto
  // Idealmente, elas também seriam ajustadas ou removidas se a autenticação for globalmente desativada
  ...
  const handleOpenMessagesModal = () => {
    fetchMessages(); // Busca as mensagens ao abrir
  ...
  const handleCloseMessagesModal = () => {
    ...
    setErrorMessages(null); // Limpa erros ao fechar
  };
